...
describe User do
  ...
  it "should reject duplicate user names" do
    User.create!(@attr)
    user_with_duplicate_name = User.new(@attr.merge(:email => 'unique@example.com'))
    expect(user_with_duplicate_name).to_not be_valid
  end

  it "should reject duplicate email addresses" do
    User.create!(@attr)
    user_with_duplicate_email = User.new(@attr.merge(:name => 'uniquename'))
    expect(user_with_duplicate_email).to_not be_valid
  end

  it "should reject user name identical up to case" do
